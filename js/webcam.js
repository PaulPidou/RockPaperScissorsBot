...
class Webcam {
  ...
  capture() {
    ...
    takepicture() {
        ...
        canvas.getContext('2d').drawImage(this.webcamElement, 0, 0, this.webcamElement.width, this.webcamElement.height);
  ...
  cropImage(img) {
    const size = 224; //Math.min(img.shape[0], img.shape[1]);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (navigator.getUserMedia) {
        navigator.getUserMedia(
            ...
            error => {
              alert('User media request denied!');
              reject(error);
            });
      } else {
        alert("Can't access the camera!");
        reject();
      }
    });
  }
